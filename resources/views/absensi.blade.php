...
<body class="bg-gradient-to-br from-indigo-50 to-blue-100 min-h-screen flex flex-col items-center justify-center">
    ...
    <div class="bg-white shadow-xl rounded-2xl p-8 w-full max-w-5xl text-center">
        ...
        <div class="relative flex flex-col items-center">
            <video id="video" width="640" height="480" autoplay muted
                class="mx-auto rounded-lg border-4 border-indigo-500 shadow-md"></video>
            <div id="cameraStatus"
                class="absolute bottom-2 right-4 bg-indigo-600 text-white text-sm px-3 py-1 rounded-full shadow-md">
                Mengaktifkan kamera...
            </div>
        </div>
    ...
    <script>
        ...
        async function loadDataSiswa() {
            try {
                const res = await fetch("{{ route('absensi.data') }}?sesi={{ $sesi }}");
            ...
            setInterval(async () => {
                ...
                try {
                    ...
                    if (data.status === "success") {
                        ...
                        statusEl.textContent = "✅ " + data.name + " sudah absen hari ini.";
                        statusEl.classList.remove("text-red-600", "text-gray-700");
                        statusEl.classList.add("text-green-600", "font-semibold");
                    } else {
                        statusEl.textContent = "❌ " + (data.message || "Wajah tidak dikenali!");
                        statusEl.classList.remove("text-green-600", "text-gray-700");
